...
This program is part of a larger project, the Open Data NC project. The goal of the project was to take
publicly available police reports, extract the data from them, and put that data in a single searchable
database, so that journalists, investigative reports, etc. could more easily access and utilize it.

The role of this program in that goal is to extract the data from the forms. The forms are available as
PDFs, so in order to contextualize the data on them, the forms are first converted into XML documents,
because we can then use the XML tags and attributes to differentiate names of fields from the data those
fields contain.

The core of the program lies in the pairFieldandData and pairCheckmarkData functions. For the most part,
extracting the data is achieved through positional comparisons of the coordinates of the various XML
tags (the 'top' and 'left' attributes) in order to match data to a certain field. This is analogous to
a human looking at the form and understanding the boxes that demarcate each field.

There are five major steps of the current algorithm:

    (Step I) ----- Read a single xml file in and store each line of the file as a single string item in
                    a list (the "GetFileLinesList" step).

    (Step II) ---- Go through the file lines list and separate it into two lists, one list for the keys
                    or Field-Items, and one list for the values or Data-Items (the "ProcessLists" step).

    (Step III) --- Sort both the list of Field-Items and the list of Data-Items by the Y Coordinate of
                    the items in each list (the "QSort" step).

    (Step IV) ---- Start matching Data-Items to Field-Items to create key-value pairs:
                    move down the form in sections, grouping Data-Items and Field-Items however is most
                    efficient but generally by either a shifting window of y coordinates, or as necessary
                    based on the matching tactics that will be used for that section.
                    Within a section, match until the group of Data-Items is exhausted, then fill out the
                    constantly growing list of key-value pairs by pairing the rest of the Field-Items from
                    that section with a value of NULL, or something more appropriate if it can be inferred.
                    Continue in this manner until the list of key-value pairs reaches its proper and
                    maximum length, therefore representing the totality of one report (the "Matching" step).

    (Step V) ----- Turn the list of key-value pairs into a dictionary so that a value can be accessed by
                    knowing the key; return the dictionary if the program was supplied a single xml file;
                    or more likely, add the dictionary to a growing list of dictionaries if the program was
                    supplied a directory of xml files. In fact, this growing list may instead itself be a
                    dictionary, so that one of the inner dictionaries representing one specific report can
                    be accessed by, say, the identification number of the report rather than a list index.
                    Return the list or dictionary when the scraper has run through the entire supplied
                    directory (the "Return" step).
'''
